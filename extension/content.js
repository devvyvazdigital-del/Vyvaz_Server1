console.log("[Vyvaz] extensão iniciada");

(function () {
  const API_BASE_URL = "https://vyvaz-server.onrender.com";

  const OVERLAY_ID = "vyvaz-overlay-root";
  const BOX_CLASS = "vyvaz-card-box";
  ...
  function injectStyles(root) {
    if (root.getElementById("vyvaz-style")) return;

    const style = document.createElement("style");
    style.id = "vyvaz-style";
    style.textContent = `
      .${BOX_CLASS} {
        position: absolute;
        width: 230px;
      ...
      .vyvaz-arrow.open { transform: rotate(90deg); }

      .vyvaz-card-green  { background: #e8f8ee; border-color: #2ea55e; color: #163d26; }
      .vyvaz-card-yellow { background: #fffbea; border-color: #d4a000; color: #584200; }
      .vyvaz-card-red    { background: #fef0f0; border-color: #d44c4c; color: #5b1f1f; }
      ...
      .vyvaz-red-text    { color: #c62828; font-weight: 700; }
      .vyvaz-green-text  { color: #1a7a42; font-weight: 700; }
      .vyvaz-yellow-text { color: #8a5c00; font-weight: 700; }

      .vyvaz-expand-all-wrap {
        position: fixed;
        top: 80px;
        right: 14px;
        z-index: 1000000;
      ...
      .vyvaz-expand-all-btn:hover { background: #237a47; }
      .vyvaz-collapse-all-btn {
        padding: 6px 12px;
        font-size: 12px;
        font-weight: 600;
        font-family: Arial, sans-serif;
      ...
      .vyvaz-collapse-all-btn:hover { background: #545b62; }
  ...
  function classeNumero(valor) {
    if (valor > 0) return "vyvaz-green-text";
    if (valor < 0) return "vyvaz-red-text";
    return "vyvaz-yellow-text";
  }

  function getCardColorClassByMc(mc) {
    if (mc <= 6) return "vyvaz-card-red";
    if (mc < 10) return "vyvaz-card-yellow";
    return "vyvaz-card-green";
  }

  function getStatusByMc(mc) {
    if (mc <= 6) return { texto: "Crítico", classe: "vyvaz-red-text" };
    if (mc < 10) return { texto: "Atenção", classe: "vyvaz-yellow-text" };
    return { texto: "Saudável", classe: "vyvaz-green-text" };
  }
  ...
  async function getSessao() {
    try {
      ...
      return {
        ...
        user: storage.vyvaz_user || storage.user || storage.venforce_user || null
      };
    } catch {
  ...
  async function loadCostsFromApi() {
    ...
    try {
      ...
      console.log("[Vyvaz] base carregada:", currentBaseId, Object.keys(COST_DB).length, "IDs");
      return true;
    } catch (err) {
      console.warn("[Vyvaz] falha ao carregar base:", err.message);
      clearLoadedBase();
      return false;
    }
  }
  ...
  async function loadCostsLocalFallback() {
    try {
      ...
      console.log("[Vyvaz] fallback local carregado");
      return true;
    } catch {
      clearLoadedBase();
      return false;
  ...
  function ensureOverlay() {
    ...
    wrap.className = "vyvaz-expand-all-wrap";

    const expandBtn = document.createElement("button");
    expandBtn.type = "button";
    expandBtn.className = "vyvaz-expand-all-btn";
    ...
    collapseBtn.className = "vyvaz-collapse-all-btn";
    ...
    scanBtn.type = "button";
    scanBtn.className = "vyvaz-expand-all-btn";
    ...
    debugBox.className = "vyvaz-debug-box";
    debugBox.style.display = "none";
    debugBox.innerHTML = `
      <div class="vyvaz-debug-total">
        <span>Escaneados</span><span id="vf-dbg-total">0</span>
      </div>
      <div class="vyvaz-debug-grid">
  ...
  function atualizarDebugNaPagina() {
    ...
    const boxes = root.querySelectorAll("[data-vyvaz-key]");
    const mcs = [];
    boxes.forEach(box => {
      const mc = box.__vyvazCache?.dados?.mc;
 ...
 function extrairFrete(row) {
  ...
  function getBoxKey(row, index) {
    ...
    return id ? `vyvaz-box-${id}-${index}` : `vyvaz-box-index-${index}`;
  }

  function ensureBox(key) {
    const root = getOverlayRoot();
    let box = root.querySelector(`[data-vyvaz-key="${key}"]`);
    if (box) return box;

    box = document.createElement("div");
    box.className = `${BOX_CLASS} vyvaz-card-green`;
    box.setAttribute("data-vyvaz-key", key);
  ...
  function renderSemCusto(box, id) {
    box.className = `${BOX_CLASS} vyvaz-card-yellow vyvaz-compact`;
    box.innerHTML = `
      <div style="font-size:8px;opacity:0.4;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${id || "—"}</div>
      <div style="font-size:9px;font-weight:700;margin-top:2px;" class="vyvaz-yellow-text">Sem custo</div>
      <div style="font-size:8px;opacity:0.38;margin-top:1px;">Base: ${currentBaseId || "nenhuma"}</div>
    `;
  }

  function renderErroExtracao(box, id) {
    box.className = `${BOX_CLASS} vyvaz-card-red vyvaz-compact`;
    box.innerHTML = `
      <div style="font-size:8px;opacity:0.4;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${id || "—"}</div>
      <div style="font-size:9px;font-weight:700;margin-top:2px;" class="vyvaz-red-text">Erro</div>
    `;
  }
  ...
  function renderBox(box, id, dados, extras = {}) {
    ...
    box.className = `${BOX_CLASS} ${cardCls}${expanded ? "" : " vyvaz-compact"}`;
    ...
    if (!expanded) {
      box.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;gap:2px;">
          ...
          <div class="vyvaz-arrow" style="flex-shrink:0;">▶</div>
        </div>
      `;
      return;
    }
    ...
    box.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
        <span style="font-size:9px;font-weight:700;opacity:0.5;letter-spacing:.04em;">VYVAZ</span>
        <div class="vyvaz-arrow open">▶</div>
      </div>
  ...
  function cleanupBoxes(validKeys) {
    getOverlayRoot().querySelectorAll(`.${BOX_CLASS}`).forEach(box => {
      if (!validKeys.has(box.getAttribute("data-vyvaz-key"))) box.remove();
    });
  }
  ...
  function processarRow(row, index) {
    ...
    if (!podeUsarCache) box.__vyvazCache = { id, dados, precoInfo };
  ...
  function startStorageWatcher() {
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (keys.some(k => k in changes)) {
        console.log("[Vyvaz] storage alterado, recarregando...");
        scheduleFullReload();
      }
    });
  }
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.action === "VYVAZ_SET_TOKEN") {
      chrome.storage.local.set({ 
        token: msg.token,
        vyvaz_user: msg.user
      }, () => {
        console.log("[Vyvaz] token salvo via login portal");
        scheduleFullReload();
      });
    }
    if (msg.action === "VYVAZ_EXPAND_ALL") {
      expandAllOnNextRender = true;
      scheduleProcess();
    }
  });
  ...
  async function init() {
    ...
    startObserver();
    startStorageWatcher();
    console.log("[Vyvaz] inicialização concluída");
  }

  chrome.storage?.local?.get(["vyvaz_ativo", "venforce_ativo"], result => {
    if ((result?.vyvaz_ativo ?? result?.venforce_ativo) === false) {
      console.log("[Vyvaz] extensão desativada pelo usuário");
      return;
    }
    init();
  });
