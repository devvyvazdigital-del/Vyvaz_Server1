const API_URL = "https://vyvaz-server.onrender.com";
const API_BASE = "https://vyvaz-server.onrender.com";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const emptyBasesBox = document.getElementById("emptyBases");
  ...
  // ==========================
  // MOCK USER (login temporariamente removido)
  // ==========================
  async function garantirUsuarioMock() {
    const storage = await getStorage(["vyvaz_user", "usuario", "user", "venforce_user", "token"]);
    const existente =
      storage.vyvaz_user || storage.usuario || storage.user || storage.venforce_user || null;

    // Garante que sempre existe um token (content.js precisa dele para buscar base)
    if (!storage.token) {
      await setStorage({ token: "vyvaz-token" });
    }

    if (existente) return existente;

    const mock = { nome: "Usuário Vyvaz" };
    await setStorage({ vyvaz_user: mock, usuario: mock });
    return mock;
  }
  ...
  async function carregarEstadoAtivo() {
    const { vyvaz_ativo, venforce_ativo } = await getStorage([
      "vyvaz_ativo",
      "venforce_ativo"
    ]);
    const ativo = (vyvaz_ativo ?? venforce_ativo) !== false;
  ...
  if (scannerToggle) {
    scannerToggle.addEventListener("change", async () => {
      ...
      await setStorage({ vyvaz_ativo: novo });
      setAppStatus(
  ...
  function showLoginView() {
    // Login removido temporariamente: mantemos a função por compatibilidade,
    // mas sempre abrimos direto no app.
  ...
  async function apiFetch(path, options = {}) {
    ...
    if (response.status === 401) {
      ...
      // Sem login: só avisamos e seguimos com UI disponível.
      throw new Error(data.erro || "Sessão inválida. Atualize o token para carregar bases.");
    }
...
console.log("LOGIN ENVIADO:", email, password);
  ...
  async function preencherUsuario() {
    const storage = await getStorage(["usuario", "vyvaz_user", "user", "venforce_user"]);
    const usuario =
      storage.usuario || storage.vyvaz_user || storage.user || storage.venforce_user || null;
...
function getBaseValue(base) {
  ...
  function renderBases(lista) {
    ...
    if (!lista.length) {
      ...
      if (emptyBasesBox) emptyBasesBox.style.display = "block";
      return;
    }

    if (emptyBasesBox) emptyBasesBox.style.display = "none";
    lista.forEach((base) => {
  ...
  async function carregarBases() {
    ...
    try {
      ...
      if (!todasAsBases.length) {
        ...
        setAppStatus("Nenhuma base carregada.", "#666");
        return;
      }
      ...
      console.error("[Vyvaz] erro ao carregar bases:", error);
      basesSelect.innerHTML = `<option value="">Nenhuma base carregada</option>`;
      if (emptyBasesBox) emptyBasesBox.style.display = "block";
      basesSelect.disabled = false;
      setAppStatus("", "#666");
    }
    ...
    try {
      ...
      function mostrarPreviewModal(payload) {
        return new Promise((resolve) => {
          ...
          meta.textContent = `Total linhas: ${payload.linhasPlanilha ?? payload.total ?? 0} • IDs detectados: ${payload.idsDetectados ?? payload.total ?? 0} • Coluna: ${payload.colunaUsada || payload.colunaId || "—"}`;
      ...
      console.error("[Vyvaz] erro ao criar base/importar planilha:", error);
  ...
  async function escanearPagina() {
    ...
    try {
      ...
      try {
        await chrome.tabs.sendMessage(tab.id, { action: "VYVAZ_EXPAND_ALL" });
      ...
      try {
        const [{ result }] = await chrome.scripting.executeScript({
          ...
          world: "MAIN",
          func: () => {
            const overlay = document.getElementById("vyvaz-overlay-root");
        ...
        console.error("[Vyvaz] erro ao escanear:", e);
        setAppStatus(
      ...
      console.error("[Vyvaz] erro ao escanear:", error);
  ...
  async function restaurarSessao() {
    try {
      showAppView();
      await garantirUsuarioMock();
      await preencherUsuario();
      await carregarBases();
    } catch (error) {
      console.error("[Vyvaz] erro ao restaurar sessão:", error);
      showAppView();
      await garantirUsuarioMock();
      await preencherUsuario();
      await carregarBases();
  ...
  // EVENTOS
  // ==========================
  // Login removido temporariamente: não registramos handlers de login.

  if (btnLogout) {
    btnLogout.addEventListener("click", async () => {
      try {
        await removeStorage(["token", "email", "baseSelecionada"]);
        setAppStatus("Sessão limpa. Você ainda pode usar o scanner.", "#666");
        await garantirUsuarioMock();
        await preencherUsuario();
        await carregarBases();
      } catch (error) {
        console.error("[Vyvaz] erro no logout:", error);
  ...
  if (btnDesabilitarBase) {
    btnDesabilitarBase.addEventListener("click", async () => {
      ...
      try {
        ...
        console.error("[Vyvaz] erro ao desabilitar base:", error);
  ...
  if (btnExcluirBase) {
    btnExcluirBase.addEventListener("click", async () => {
      ...
      try {
        ...
        console.error("[Vyvaz] erro ao excluir base:", error);
  ...
  if (expandAllBtn) {
    expandAllBtn.addEventListener("click", async () => {
      try {
        ...
        console.warn("[Vyvaz] Expandir todos:", err);
